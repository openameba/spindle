...
### セットアップ
このパッケージはPlaywrightを使用したアクセシビリティテストを含んでいます。初回セットアップ時は、ルートディレクトリで`yarn install`を実行してください。これにより、必要なPlaywrightブラウザが自動的にインストールされます。

もしテストが「Executable doesn't exist」エラーで失敗する場合は、以下のコマンドを実行してください：

```bash
cd packages/spindle-syntax-themes
