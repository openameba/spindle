...
- 画面内の重要度に応じて`variant`を使い分け、主要導線には`contained`、補助導線には`outlined`/`neutral`を選択します
- 用途に応じてサイズを使い分けます
  - `large`: ページの主要導線として強調したいCTA
  - `medium`: セクション内の標準的な遷移導線
  - `small`: 密度の高いUI内で補助的に配置する遷移導線
- ページ幅に合わせて広がるCTAが必要な場合は`layout="fullWidth"`を使います
- アイコンを併用する場合は、リンクラベルを補助する文脈で使用します
  - `iconPosition="start"`: 遷移の意味を先頭で補強したい場合
  - `iconPosition="end"`:「次へ」「もっと見る」など進行方向を末尾で示したい場合
  - アイコンの余白はコンポーネントのサイズに応じた既定値（large: 6px / medium: 4px / small: 2px）を維持し、視認性と可読性を優先します
