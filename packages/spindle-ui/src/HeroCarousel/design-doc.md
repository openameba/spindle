...
自動再生の有無を用途に応じて選択し、2件以上のアイテムを渡します。なるべく16文字未満のタイトルにしてください（2行以上は省略表示されます）。
...
- [フォーカスを見えるようにする](https://a11y-guidelines.ameba.design/2/4/7/)
  - [ ] フォーカスリングは表示されている
- [フォーカスの外観を確保する](https://waic.jp/translations/WCAG22/Understanding/focus-appearance.html)
  - [ ] フォーカスリングは背景色に対して3:1以上のコントラスト比かつ2pxの太さのアウトラインで可視化されている
