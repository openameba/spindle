...
<Meta of={TableStories} summary="構造化データを表形式で表示するコンポーネント。borderTypes、layout（auto/fixed/scrollable）、striped、rounded、セル結合、CSS Variablesによるカスタマイズをサポート。" />
...
`layout` propでテーブルのレイアウト方式を指定できます。`"auto"` と `"fixed"` はCSSの `table-layout` プロパティに対応しています。

### 自動レイアウト

`layout="auto"` はデフォルトの動作で、コンテンツに応じて列幅が自動調整されます。`layout` propを省略した場合も同じ動作になります。

#### Auto Layout

<Story of={TableStories.AutoLayout} />

<Source
  code={`<Table.Frame borderTypes={['horizontal']} layout="auto">
  <Table.Header>
    <Table.Row>
  ...
  <Table.Body>
    <Table.Row>
      <Table.Head scope="row">商品A</Table.Head>
      <Table.Cell>高品質な商品Aの詳細説明文です。コンテンツに応じて列幅が自動調整されます。</Table.Cell>
      <Table.Cell>¥1,200,000</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table.Frame>`}
