...
AppealModalは、ユーザーに対して重要なお知らせや情報を伝えるためのモーダルコンポーネントです。

HTMLDialogElementを利用しています。
画像・タイトル・本文・ボタンなどの構成要素を組み合わせて使用します。特定用途での利用を想定しているため、カスタマイズ性は低く抑えられていますが、必要に応じて独自要素を追加できます。

モバイルとデスクトップでレイアウトが自動的に切り替わり、レスポンシブに対応しています。デスクトップではIconButtonでの閉じる操作、モバイルではTextButtonでの閉じる操作が表示されます。
...
- Escapeキーで閉じる動作が機能するか（PlaywrightなどのE2Eテストツールが必要）
