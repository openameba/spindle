import { Meta, Story, Source, Description } from '@storybook/addon-docs/blocks';
import { actions } from '@storybook/addon-actions';
import { InlineDropDown } from './InlineDropDown';

# InlineDropDown

<Meta title="Form/InlineDropDown" component={InlineDropDown} />

![stability-experiment](https://img.shields.io/badge/stability-experiment-red.svg)

アイテム一覧画面の並び替え等の目的で利用されるDropDown型のコンポーネントです。一般的なフォーム画面の選択UIで利用することは想定していません。
InlineDropDownは試験的なコンポーネントであるため、今後に破壊的な変更や削除がおこなわれる可能性があることに注意して利用してください。

<Source
  language="javascript"
  code={`import { InlineDropDown } from '@openameba/spindle-ui/Form'`}
/>

<Source
  language="css"
  code={`@import './node_modules/@openameba/spindle-ui/Form/InlineDropDown.css'`}
/>

<Source
  language="html"
  code={`<link rel="stylesheet" href="https://unpkg.com/@openameba/spindle-ui/Form/InlineDropDown.css">`}
/>

<Description>
  このコンポーネントの利用にはJavaScriptでの処理が必要なため、HTMLのみでは正常に動作しません。[React版](https://github.com/openameba/spindle/tree/main/packages/spindle-ui/src/Form/InlineDropDown.tsx)を参考に実装してください。
</Description>

## Medium

<Preview withSource="open">
  <Story name="Medium">
    <InlineDropDown
      aria-label="Fruits"
      name="fruits"
      {...actions('onClick', 'onChange', 'onInput', 'onFocus', 'onBlur')}
    >
      <option value="apple">Apple</option>
      <option value="green-apple">Green Apple</option>
      <option value="melon">Melon</option>
    </InlineDropDown>
  </Story>
</Preview>

<Source
  code={`
<InlineDropDown aria-label="Fruits" name="fruits">
  <option value="apple">Apple</option>
  <option value="orange">Orange</option>
  <option value="melon">Melon</option>
</InlineDropDown>
`}
/>

<Source
  language="html"
  code={`
<label class="spui-InlineDropDown-label is-active">
  <span class="spui-InlineDropDown-visual">
    <span class="spui-InlineDropDown-text spui-InlineDropDown-text--medium">Apple</span>
    <span class="spui-InlineDropDown-icon spui-InlineDropDown-icon--medium"><svg/></span>
  </span>
  <select class="spui-InlineDropDown-select spui-InlineDropDown-select--medium" aria-label="Fruits" name="fruits"
  >
    <option value="apple">Apple</option>
    <option value="green-apple">Green Apple</option>
    <option value="melon">Melon</option>
  </select>
  <span class="spui-InlineDropDown-outline"></span>
</label>
  `}
/>

## Small

<Preview withSource="open">
  <Story name="Small">
    <InlineDropDown
      visualSize="small"
      aria-label="Fruits"
      name="fruits"
      {...actions('onClick', 'onChange', 'onInput', 'onFocus', 'onBlur')}
    >
      <option value="apple">Apple</option>
      <option value="green-apple">Green Apple</option>
      <option value="melon">Melon</option>
    </InlineDropDown>
  </Story>
</Preview>

<Source
  code={`
<InlineDropDown visualSize="small" aria-label="Fruits" name="fruits">
  <option value="apple">Apple</option>
  <option value="orange">Orange</option>
  <option value="melon">Melon</option>
</InlineDropDown>
`}
/>

<Source
  language="html"
  code={`
<label class="spui-InlineDropDown-label is-active">
  <span class="spui-InlineDropDown-visual">
    <span class="spui-InlineDropDown-text spui-InlineDropDown-text--small">Apple</span>
    <span class="spui-InlineDropDown-icon spui-InlineDropDown-icon--small"><svg/></span>
  </span>
  <select class="spui-InlineDropDown-select spui-InlineDropDown-select--small" aria-label="Fruits" name="fruits"
  >
    <option value="apple">Apple</option>
    <option value="green-apple">Green Apple</option>
    <option value="melon">Melon</option>
  </select>
  <span class="spui-InlineDropDown-outline"></span>
</label>
  `}
/>

## Selected And Disabled

<Description>
  select要素と同様に、selectedやdisabled属性を付与したoption要素にも対応しています。
</Description>

<Preview withSource="open">
  <Story name="Selected And Disabled">
    <InlineDropDown
      aria-label="Fruits"
      name="fruits"
      {...actions('onClick', 'onChange', 'onInput', 'onFocus', 'onBlur')}
    >
      <option value="apple">Apple</option>
      <option value="orange" selected disabled>
        Orange
      </option>
      <option value="melon">Melon</option>
    </InlineDropDown>
  </Story>
</Preview>

<Source
  code={`
<InlineDropDown aria-label="Fruits" name="fruits">
  <option value="apple">Apple</option>
  <option value="orange" selected disabled>Orange</option>
  <option value="melon">Melon</option>
</InlineDropDown>
  `}
/>

<Source
  language="html"
  code={`
<label class="spui-InlineDropDown-label">
  <span class="spui-InlineDropDown-visual">
    <span class="spui-InlineDropDown-text">Apple</span>
    <span class="spui-InlineDropDown-icon">
      <svg>...</svg>
    </span>
  </span>
  <select class="spui-InlineDropDown-select" aria-label="Fruits" name="fruits">
    <option value="apple">Apple</option>
    <option value="green-apple" disabled>Green Apple</option>
    <option value="melon">Melon</option>
  </select>
  <span class="spui-InlineDropDown-outline"></span>
</label>
  `}
/>

## Accessibility

<Description>
  aria-label属性やlabel要素で適切なラベリングをしてください。
</Description>

<Source
  code={`
<InlineDropDown aria-label="Fruits" id="fruits-selection" name="fruits" placeholder="Fruits">
  <option value="apple">Apple</option>
  <option value="orange">Orange</option>
  <option value="melon">Melon</option>
</InlineDropDown>
  `}
/>

<Source
  code={`
<>
  <InputLabel id="fruits-selection">Fruits</InputLabel>
  <InlineDropDown id="fruits-selection" name="fruits" placeholder="Fruits">
    <option value="apple">Apple</option>
    <option value="orange">Orange</option>
    <option value="melon">Melon</option>
  </InlineDropDown>
</>
  `}
/>
