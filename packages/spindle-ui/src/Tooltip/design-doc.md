...
FigmaではTooltip（モードレス形式）とToggletip（モード形式）の2種類が定義されていますが、現時点ではTooltip（モードレス形式）のみを実装します。
Toggletip（モード形式）はTooltipコンポーネントとしての対応予定はありません。

Tooltipコンポーネントは、表示方法によって2つのモードをサポートします：
