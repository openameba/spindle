...
通常は`<BreadcrumbItem>`と一緒に使います。現在地には`current`を指定し、リンクではなく非インタラクティブ要素（`<span aria-current="page">`）で表現します。
...
<nav class="spui-Breadcrumb" aria-label="パンくずリスト">
  <ol class="spui-Breadcrumb-list">
    ...
    <li class="spui-Breadcrumb-item"><span aria-current="page">Amebaとは</span><svg class="spui-Breadcrumb-chevron" role="img" aria-hidden="true"></svg></li>
  </ol>
</nav>
  `}
/>
...
`<BreadcrumbItem>`を使わず、直接`<a>`要素を指定できます。各種フレームワークで定義された`<Link>`も同様に指定できます。その際には、現在地はリンクではなく`<span aria-current="page">`で表現するか、`<a aria-current="page">`を直接書く場合は`BreadcrumbItem`を使わないパターンに限定してください。
