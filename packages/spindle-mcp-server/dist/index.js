...
function getTokenTypes() {
    ...
    const types = fs_1.default.readdirSync(tokenDir)
...
const TokenType = zod_1.z.enum(getTokenTypes());
...
function generateIconSampleCode(iconName) {
...
function parseCssColorTokens(css) {
    const tokens = {};
...
// アイコンの使用例コードを取得するツール
server.tool("get_icon_sample_code", "指定したアイコンの使用例コードを取得します", {
    name: zod_1.z.string().describe("アイコン名")
}, async ({ name }) => {
...
// CSSカラートークンを取得するツール
server.tool("get_css_color_tokens", "CSSで定義されたカラートークンを取得する", {}, () => {
...
// Figmaデータを抽出する関数
function extractFigmaUrl(figmaFileContent) {
...
async function getComponentFigmaData(componentName) {
...
async function getAllComponentsFigmaData() {
...
// Figmaデータを取得するツール
server.tool("get_component_figma_url", "指定したコンポーネントのFigmaデザインデータURLを取得する", {
    name: zod_1.z.string().describe("コンポーネント名")
}, async ({ name }) => {
...
server.tool("get_all_components_figma_urls", "全てのコンポーネントのFigmaデザインデータURLを取得する", {}, async () => {
